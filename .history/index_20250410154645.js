...
app.post("/api/waste/add", async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO waste_entries (description, category, quantity, image_url, user_id)
       VALUES (?, ?, ?, ?, ?)`,
      [description, category, quantity, image_url || null, req.session.user_id]
    );

    res.status(201).json({
      message: "Waste entry added successfully",
      wasteEntryId: result.insertId
    });
  } catch (error) {
    console.error("Error adding waste entry:", error);
    res.status(500).json({ message: "Error adding waste entry" });
  }
});
