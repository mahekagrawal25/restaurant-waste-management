...
app.get("/api/pickup-requests", authenticateToken, async (req, res) => {
  ...
  try {
    const [requests] = await pool.query(`
      SELECT wc.id, wc.description, wc.status, wc.pickup_date, u.username AS requested_by, we.category, we.quantity
...
app.put("/api/mark-collected/:id", authenticateToken, async (req, res) => {
  if (req.user.role !== 'waste_collector') {
    return res.status(403).json({ message: "Access denied" });
  }
  ...
  try {
    await pool.query(
      `UPDATE waste_collection SET status = 'Collected', pickup_date = NOW(), collector_name = ? WHERE id = ?`,
