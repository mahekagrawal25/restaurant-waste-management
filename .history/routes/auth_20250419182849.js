...
router.get('/waste-entries', authenticateToken, async (req, res) => {
  try {
      const [wasteEntries] = await pool.query(
      ...
      if (wasteEntries.length === 0) {
          return res.status(404).json({ message: 'No waste entries found.' });
      }
...
// Route to request a waste collection
router.post('/request', authenticateToken, async (req, res) => {
  const { description, pickup_date, collector_name, waste_entry_id } = req.body; // Include waste_entry_id

  // Check if the user provided all required details
  if (!description || !pickup_date || !collector_name || !waste_entry_id) {  // Added validation for waste_entry_id
      return res.status(400).json({ message: 'Please fill all fields.' });
  }

  try {
      // Insert the waste collection request into the database
      const [result] = await pool.query(
          'INSERT INTO waste_collection (description, pickup_date, collector_name, user_id, waste_entry_id) VALUES (?, ?, ?, ?, ?)',  // Added waste_entry_id to query
          [description, pickup_date, collector_name, req.user.id, waste_entry_id]  // Added waste_entry_id to the query parameters
      );

      res.status(201).json({ message: 'Collection requested successfully.' });
  } catch (error) {
