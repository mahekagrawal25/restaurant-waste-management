...
router.get('/waste-entries', auth, async (req, res) => {
  try {
    const entries = await WasteEntry.find().sort({ created_at: -1 });
    res.json(entries);
  } catch (err) {
...
router.get('/food-donations', auth, async (req, res) => {
  try {
    const donations = await FoodDonation.find().sort({ created_at: -1 });
    res.json(donations);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/waste-collection', auth, async (req, res) => {
  try {
    const collections = await WasteCollection.find().sort({ created_at: -1 });
