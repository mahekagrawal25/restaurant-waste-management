...
router.post('/request', authenticateToken, async (req, res) => {
  const { description, pickup_date, collector_name } = req.body;

  // Check if the user provided all required details
  if (!description || !pickup_date || !collector_name) {
  ...
  try {
      ...
      const [result] = await pool.query(
          'INSERT INTO waste_collection (description, pickup_date, collector_name, user_id) VALUES (?, ?, ?, ?)', 
          [description, pickup_date, collector_name, req.user.id]
      );
...
// GET /api/waste-entries/my-entries
router.get('/my-entries', authenticateToken, async (req, res) => {
  try {
    const [entries] = await pool.query(
      'SELECT id, description, category, quantity FROM waste_entries WHERE user_id = ?',
      [req.user.id]
    );
