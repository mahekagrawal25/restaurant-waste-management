...
router.get('/waste-collection/entries', authenticateToken, async (req, res) => {
  try {
      const [wasteEntries] = await pool.query(
...
router.post('/request', authenticateToken, async (req, res) => {
  ...
  if (!waste_entry_id) {
      return res.status(400).json({ message: 'Invalid request, waste entry ID is required.' });
  }

  try {
      // Insert request into the waste_collection table
      const [result] = await pool.query(
          ...
          [waste_entry_id, req.user.id, waste_entry_id]
      );
