...
router.post('/request', authenticateToken, async (req, res) => {
  const { waste_entry_id } = req.body;

  // Ensure waste_entry_id is provided
  if (!waste_entry_id) {
      return res.status(400).json({ message: 'Please select a waste entry.' });
  }

  try {
      // Insert a new collection request into the database
      const [result] = await pool.query(
          'INSERT INTO waste_collection (description, status, user_id, waste_entry_id) VALUES (?, ?, ?, ?)',
          ['Requested', 'Pending', req.user.id, waste_entry_id]
      );

      res.status(201).json({ message: 'Collection request successfully submitted.' });
  } catch (error) {
