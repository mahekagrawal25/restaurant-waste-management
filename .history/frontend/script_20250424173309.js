...
async function fetchPendingRequests() {
  const token = localStorage.getItem("token");
  const response = await fetch("http://localhost:5000/api/get-pending-requests", {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${token}`,
    },
  ...
  if (response.ok) {
    ...
    data.data.forEach((request) => {
      ...
      row.innerHTML = `
        ...
        <td><button class="mark-collected" data-id="${request.id}" data-description="${request.description}">Mark as Collected</button></td>
      `;
      tableBody.appendChild(row);
    });

    // Add event listeners to "Mark as Collected" buttons
    document.querySelectorAll(".mark-collected").forEach(button => {
      button.addEventListener("click", async (e) => {
        const requestId = e.target.getAttribute("data-id");
        const description = e.target.getAttribute("data-description");

        // Prompt the collector for their name
        const collectorName = prompt("Enter your name:");

        if (collectorName) {
          await markAsCollected(requestId, collectorName);
        }
      });
    });
  } else {
...
async function markAsCollected(waste_entry_id, collector_name) {
  const token = localStorage.getItem("token");

  const response = await fetch("http://localhost:5000/api/mark-as-collected", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`,
  ...
  if (response.ok) {
    alert("Pickup marked as collected successfully!");
