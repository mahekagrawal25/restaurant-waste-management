...
document.getElementById("donationForm")?.addEventListener("submit", async (e) => {
  e.preventDefault();
  const token = localStorage.getItem("token");
  const statusMsg = document.getElementById("donationStatus");

  const food_name = document.getElementById("food_name").value.trim();
  const quantity = document.getElementById("donation_quantity").value.trim();
  ...
  try {
    const res = await fetch("http://localhost:5000/api/food-donations", {
      ...
      body: JSON.stringify({ food_name, quantity, expiry_date }),
    });
