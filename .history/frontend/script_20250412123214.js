...
document.getElementById("addWasteForm")?.addEventListener("submit", async (event) => {
  event.preventDefault();

  const description = document.getElementById("description").value.trim();
  const category = document.getElementById("category").value;
  const quantity = parseInt(document.getElementById("quantity").value);
  const image_url = document.getElementById("image_url").value.trim() || null;  // Optional image URL
  const token = localStorage.getItem("token");

  // Validate form fields
  if (!description || !category || !quantity) {
    alert("All fields are required except image URL.");
    return;
  }
  ...
  try {
    const response = await fetch("http://localhost:5000/api/waste/addWaste", {
      ...
      body: JSON.stringify({ description, category, quantity, image_url })
    });
    ...
    if (response.ok) {
      ...
      alert(data.message || "Failed to add waste. Please try again.");
    }
  } catch (error) {
