...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fetch user's waste entries to populate the dropdown
  try {
    ...
    if (response.ok && wasteEntries.length > 0) {
      const selectElement = document.getElementById("waste_entry_id");
      wasteEntries.forEach(entry => {
        const option = document.createElement("option");
        option.value = entry.id;
        option.textContent = entry.description;  // Assuming 'description' is available in the entry
        selectElement.appendChild(option);
      });
    } else {
      alert("No waste entries available.");
    }
  } catch (error) {
    console.error("Error fetching waste entries:", error);
    alert("Failed to fetch waste entries.");
  }
});
