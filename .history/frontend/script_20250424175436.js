...
document.addEventListener("DOMContentLoaded", () => {
  const pickupContainer = document.getElementById("pickupRequests");

  // Fetch pending pickup requests
  async function loadPickupRequests() {
    try {
      const res = await fetch("/api/waste-collection/pending", {
        headers: {
      ...
      if (pickups.length === 0) {
        pickupContainer.innerHTML = `<p class="status-msg">No pending pickup requests.</p>`;
        return;
      }

      pickups.forEach((pickup) => {
        const requestCard = document.createElement("div");
        requestCard.className = "pickup-card";

        requestCard.innerHTML = `
          <h3>${pickup.description}</h3>
          <p><strong>Category:</strong> ${pickup.category}</p>
          <p><strong>Quantity:</strong> ${pickup.quantity}</p>
          <p><strong>Requested On:</strong> ${new Date(pickup.created_at).toLocaleString()}</p>
          <button class="btn-collect" data-id="${pickup.id}">
            Mark as Collected
          </button>
        `;

        pickupContainer.appendChild(requestCard);
      });
    } catch (err) {
      console.error("Error loading pickup requests:", err);
      pickupContainer.innerHTML = `<p class="status-msg error">Unable to load pickup requests.</p>`;
    }
  }
  ...
  pickupContainer.addEventListener("click", async (e) => {
    if (e.target.classList.contains("btn-collect")) {
      ...
      try {
        const res = await fetch(`/api/waste-collection/mark-collected/${id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${localStorage.getItem("token")}`,
