...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!description || !category || !quantity || quantity <= 0) {
      alert("Please fill in all required fields correctly.");
      return;
    }

    try {
      const response = await fetch("/api/waste/add", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`❌ Failed to add waste: ${data.message}`);
      }
    } catch (error) {
      console.error("Add Waste Error:", error);
      alert("🚫 An error occurred while adding waste.");
    }
  });
});
