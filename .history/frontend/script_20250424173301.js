...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fetch pending pickup requests from the backend
  const fetchPendingRequests = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/get-pending-requests", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        data.data.forEach((request) => {
          ...
          row.innerHTML = `
            <td>${request.waste_entry_id}</td>
            <td>${request.description}</td>
            <td>${request.status}</td>
            <td>${request.pickup_date || "-"}</td>
            <td>${request.collector_name || "-"}</td>
            <td>
              <button class="btn-collect" onclick="markAsCollected(${request.id})">
                Mark as Collected
              </button>
            </td>
          `;
  ...
  // Function to mark a waste entry as collected
  window.markAsCollected = async (wasteEntryId) => {
    const collectorName = prompt("Enter your name as the collector:");
    if (!collectorName) {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/mark-as-collected", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        statusMsg.textContent = data.message;
        statusMsg.className = "status-msg success";
        // Refresh the list after marking as collected
