...
document.addEventListener("DOMContentLoaded", async () => {
  if (!window.location.pathname.includes("pickup-requests.html")) return;

  const token = localStorage.getItem("token");
  const tbody = document.getElementById("pickupRequestsTableBody");

  try {
    const res = await fetch("http://localhost:5000/api/collector/pickup-requests", {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (Array.isArray(requests) && requests.length > 0) {
      requests.forEach((req) => {
        ...
        row.innerHTML = `
          ...
          <td>${req.waste_entry_id}</td>
          <td>${req.requested_by}</td>
          <td>${req.status}</td>
          <td>
            <button class="btn-submit" onclick="markAsCollected(${req.id})">
              Mark as Collected
            </button>
          </td>
        `;
...
async function markAsCollected(id) {
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/collector/mark-collected/${id}`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` },
    });
