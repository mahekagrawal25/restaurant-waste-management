...
// ✅ New Protected Route: Summary Report (Total Waste, Total Donations, Common Category)
app.get("/api/reports/summary", authenticateToken, async (req, res) => {
  try {
    ...
    if (wasteRows.length > 0) {
      totalWaste = wasteRows.reduce((sum, row) => sum + row.total_quantity, 0);

      wasteRows.forEach(row => {
        wasteByCategory.labels.push(row.category);
        wasteByCategory.values.push(row.total_quantity);
      });

      // Find the category with the maximum waste
      commonCategory = wasteRows.reduce((prev, current) => 
