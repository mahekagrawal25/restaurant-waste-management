...
app.post("/api/food-donations", authenticateToken, async (req, res) => {
  ...
  if (!food_name || !quantity || !expiry_date) {
    return res.status(400).json({ message: "All fields are required" });
  }

  try {
    await pool.query(
      `INSERT INTO food_donations (food_name, quantity, expiry_date, user_id)
       VALUES (?, ?, ?, ?)`,
      [food_name, quantity, expiry_date, user_id]
    );
...
router.post('/api/food-donations', async (req, res) => {
  const { description, quantity, donor_name, contact } = req.body;
  const user_id = req.session.userId; // assuming login stores userId in session
