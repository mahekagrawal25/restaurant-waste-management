...
// ✅ New Protected Route: Summary Report
app.get("/api/reports/summary", authenticateToken, async (req, res) => {
  try {
    ...
    wasteRows.forEach(row => {
      totalWaste += row.total_quantity;
      labels.push(row.category);
      values.push(row.total_quantity);
      if (row.total_quantity > maxQuantity) {
        maxQuantity = row.total_quantity;
    ...
    const totalDonations = donationRows[0].total_donations;
