...
// ✅ Fetch Pending Pickup Requests (for collectors)
app.get("/api/get-pending-requests", authenticateToken, async (req, res) => {
  try {
    // Assuming the user is a collector, fetch all pending waste collection requests
    const [requests] = await pool.query(
      `SELECT wc.id, wc.description, wc.status, wc.created_at 
...
// ✅ Mark Pickup as Collected (for collectors)
app.post("/api/mark-as-collected", authenticateToken, async (req, res) => {
  const { waste_entry_id, collector_name } = req.body;

  // Validate required fields
  if (!waste_entry_id || !collector_name) {
    return res.status(400).json({ message: "Waste entry ID and collector name are required" });
  }

  try {
    // Update the waste collection status to 'Collected' and set pickup date
    await pool.query(
      `UPDATE waste_collection 
       SET status = 'Collected', pickup_date = NOW(), collector_name = ? 
       WHERE waste_entry_id = ? AND status = 'Pending'`,
      [collector_name, waste_entry_id]
    );

    res.json({ message: "Pickup marked as collected successfully" });
  } catch (error) {
