...
const cors = require("cors"); //frontend and backend can communicate
...
app.get("/api/dashboard", authenticateToken, async (req, res) => {
  try {
    const [waste] = await pool.query(
      "SELECT COUNT(*) AS totalWaste FROM waste_entries"
    );
    const [donations] = await pool.query(
      "SELECT COUNT(*) AS totalDonations FROM food_donations"
    );
    const [collections] = await pool.query(
      "SELECT COUNT(*) AS totalCollections FROM waste_collection"
    );
...
app.get("/api/recent-activities", authenticateToken, async (req, res) => {
  try {
    ...
    //Fetches last 5 waste entries added by the user, labels them as "Waste"
    const [waste] = await pool.query(
...
app.post("/api/request-collection", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Collection request submitted successfully" });
  } catch (error) {
...
app.post(
  "/api/waste-collection/mark-collected/:id",
  authenticateToken,
  async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Marking collection ${collectionId} as collected by ${collectorName}`
      );
...
// ✅ Mark Donation as Collected
app.post(
  "/api/food-donations/mark-collected/:id",
  authenticateToken,
  async (req, res) => {
...
app.get("/api/reports/waste", authenticateToken, async (req, res) => {
  try {
    ...
    `,
      [req.user.id]
    ); // Only the logged-in user's waste
...
app.get("/api/reports/donations", authenticateToken, async (req, res) => {
  try {
    ...
    `,
      [req.user.id]
    ); // Only the logged-in user's donations
